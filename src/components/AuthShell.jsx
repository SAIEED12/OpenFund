...
const AuthShell = ({ title, subtitle, children, footer }) => (
  <main className="flex flex-1 items-stretch justify-center bg-background px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
    <div className="grid w-full max-w-5xl overflow-hidden rounded-[2rem] border border-border bg-card card-shadow lg:grid-cols-[0.95fr_1.05fr]">
      <aside className="ink-panel relative hidden flex-col justify-between p-9 text-[#F6F8F5] lg:flex">
        ...
        <div>
          <p className="font-mono text-[11px] tracking-[0.18em] text-[#3DDC97] uppercase">
            Public ledger · live
          </p>
          ...
          <ul className="mt-6 space-y-3">
            {[
              ...
              >
                <BadgeCheck className="size-4 shrink-0 text-[#3DDC97]" />
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-white">{line}</p>
                  <p className="font-mono text-[11px] text-white/45">{hash}</p>
                </div>
              </li>
            ))}
          </ul>
        ...
        <p className="flex items-center gap-2 text-xs text-white/50">
          <ShieldCheck className="size-4 text-[#C9A86A]" />
